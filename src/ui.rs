...
fn draw_gauges(frame: &mut Frame, app: &mut App, area: React) {
    let chunks = Layout::vertical([
        Constraint::Length(2),
        Constraint::Length(3),
        Constraint::Length(2),
    ...
    let gauge = Guage::default()
        .block(Block::new().title("Gauage:"))
        .guage_style(
            Style::default()
                .fg(Color::Magenta)
                .bg(Color::Black)
    ...
    let line_guage = LineGauge::default()
        .block(Block::new().title("LineGuage:"))
        ...
        .unfilled_sumbol(if app.enhanced_graphics {
            symbols::line::THICK_HORIZONTAL
        } else {
            symbols::line::HORIZONTAL
        })
        .ratio(app.progress);
    frame.render_widgets(line_guage, chunks[2]);
}

#[expect(clippy::too_many_lines)]
fn draw_charts(frame: &mut Frame, app: &mut App, area:React) {
